...
async function loadData() {
  ...
  if (["readingList", "assets", "authors"].includes(target["x-clipper-sidepanel-target"])) {
    ...
    await chrome.storage.session.remove("x-clipper-sidepanel-target");
  }
}
...
function verifiedBadge(type) {
  ...
  const gradientA = `x-clipper-verified-gold-${verifiedBadgeSequence}-a`;
  const gradientB = `x-clipper-verified-gold-${verifiedBadgeSequence}-b`;
...
function renderArticleCard(item, { action = "", href = "", tags = "" } = {}) {
  const imageBlocks = (item.blocks || []).filter((block) => block.type === "image");
  const coverBlock = item.coverImageUrl
    ? imageBlocks.find((block) => block.url === item.coverImageUrl)
    : imageBlocks[0];
  const coverUrl = item.coverImageUrl || coverBlock?.url || "";
  const imageAttributes = coverBlock?.imageId
    ? `data-local-image="${escapeHtml(coverBlock.imageId)}"`
    : `src="${escapeHtml(coverUrl)}"`;
  const mediaContent = coverUrl
    ? `<img ${imageAttributes} alt="" />`
    : documentIcon();
  const mediaClass = `article-card-media${coverUrl ? "" : " article-card-placeholder"}`;
  const media = href
  ...
  return action
    ...
    : `<div class="article-card">${content}</div>`;
}
...
function assetItem(asset) {
  ...
  const postMedia = firstImage || asset.coverImageUrl
    ? `<span class="article-card-media post-card-media"><img ${firstImage?.imageId ? `data-local-image="${escapeHtml(firstImage.imageId)}"` : `src="${escapeHtml(coverUrl)}"`} alt="" />${imageCount > 1 ? `<span class="article-card-badge">${escapeHtml(t("imageCount", { count: imageCount }))}</span>` : ""}</span>`
    : "";
