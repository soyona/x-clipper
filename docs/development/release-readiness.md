# X Article Clipper 1.1.1 发布前风险检查
...
- 产品身份：Manifest 显示名为 `X Article Clipper`，短名称为 `X Clipper`，版本为 `1.1.1`；技术项目名与仓库元数据为 `x-clipper` 和 `soyona/x-clipper`。
...
只有所有 P0 项通过，且发布者明确接受或关闭 P1 项后，才应打包并提交 1.1.1。任何权限、主机范围、数据处理或品牌资产变化都必须重新运行本检查和完整测试。
