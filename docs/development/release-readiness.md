# X Article Clipper 1.0.0 发布前风险检查
...
- 产品身份：Manifest 显示名为 `X Article Clipper`，短名称为 `X Clipper`，版本为 `1.0.0`；技术项目名与仓库元数据为 `x-clipper` 和 `soyona/x-clipper`。
- 品牌资产：应用图标和页面入口使用用户确认的黑白“圆形书写轨迹 + 正在书写的铅笔”，不包含已知 X 官方 Logo path；`16/32/48/128px` PNG 与确定性 SVG 同步交付。
- 权限：仅保留运行时使用的 `storage`、`sidePanel`、`scripting`；未使用的 `activeTab` 已移除。主机范围未扩大，只覆盖四个既有 X/Twitter HTTPS 入口。
...
- P0：使用当前 X Web 页面逐项验收 Article 列表、Article 详情、Post 详情与普通 Post 列表排除规则。X DOM 属于第三方可变依赖，静态测试不能证明兼容性。
...
- P2：创建 GitHub 仓库后再初始化全新 Git 历史，并确认默认分支、LICENSE、支持渠道和发布标签策略；当前阶段按要求不初始化、不提交、不推送。

## 发布门禁

只有所有 P0 项通过，且发布者明确接受或关闭 P1 项后，才应打包并提交 1.0.0。任何权限、主机范围、数据处理或品牌资产变化都必须重新运行本检查和完整测试。
